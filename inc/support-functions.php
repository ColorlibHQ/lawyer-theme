...
function lawyer_post_cats() {
	...
	if ( $cats ) {
		...
		$categories .= '<ul class="nav"><li><span><i class="fa fm fa-th-list"></i>' . esc_html( 'Catagory :', 'lawyer' ) . '</span></li>';
...
// Blog categories
function lawyer_featured_post_cat(){
...
// social media
if ( ! function_exists( 'lawyer_social' ) ) {
	function lawyer_social( $args = array() ) {
		...
		$url = lawyer_opt( 'lawyer_social_url' );
...
// header cart count
function lawyer_cart_count( $class = '' ) {

	?>
	<div class="header-wrapicon2 <?php echo esc_attr( $class ); ?>">
		<img src="<?php echo esc_url( LAWYER_DIR_ASSETS_URI ); ?>img/icons/icon-header-02.png" class="header-icon1 js-show-header-dropdown" alt="<?php esc_attr_e( 'ICON', 'lawyer' ); ?>">
...
function lawyer_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'lawyer_contact7_form_content', 10, 2 );