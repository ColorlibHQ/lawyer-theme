...
final class Lawyer {
	...
	private function setup() {
		...
		$enqueu          = new lawyer_Enqueue();
		$enqueu->scripts = $this->enqueue();
		$enqueu->lawyer_scripts_enqueue_init();
	...
	public function support() {
		// content width
		$GLOBALS['content_width'] = apply_filters( 'lawyer_content_width', 751 );


		// text domain for translation.
		load_theme_textdomain( 'lawyer', LAWYER_DIR_PATH . '/languages' );
		...
		// Site logo size
		add_image_size( 'lawyer_site_logo_172x57', 172, 57, true );

		// Welcome thumb size
		add_image_size( 'lawyer_welcome_thumb_1820x800', 1820, 800, true );

		// About & Lawyers thumb size
		add_image_size( 'lawyer_about_thumb_280x280', 280, 280, true );
		add_image_size( 'lawyer_about_signature_thumb_199x66', 199, 66, true );

		// Practice thumb size
		add_image_size( 'lawyer_practice_thumb_448x500', 448, 500, true );

		// Testimonial Author thumb size
		add_image_size( 'lawyer_testimonial_author_thumb_35x35', 35, 35, true );

		// Appointment left thumb size
		add_image_size( 'lawyer_appointment_left_thumb_459x786', 459, 786, true );

		// Latest post thumbnail size
		add_image_size( 'lawyer_widget_post_thumb', 80, 80, true );

		// Single blog post image size
		add_image_size( 'lawyer_single_blog_750x375', 750, 375, true );
		add_image_size( 'lawyer_np_thumb', 60, 60, true );
		...
		register_nav_menus( array(
			'primary-menu' 	=> esc_html__( 'Primary Menu', 'lawyer' ),
			'practice-area' => esc_html__( 'Practice Area', 'lawyer' ),
			'useful-links' 	=> esc_html__( 'Useful Links', 'lawyer' ),
		) );
	...
	private function enqueue() {
		...
		$cssPath = LAWYER_DIR_CSS_URI;
		$jsPath  = LAWYER_DIR_JS_URI;
		...
		$scripts = array(
			'style'   => array(
				...
				array(
					...
					'dependency' => array(),
					'version'    => $this->lawyer_version,
				),
				array(
					'handler' => 'lawyer-style',
	...
	private function google_font() {
		...
		if ( 'off' !== _x( 'on', 'Google font: on or off', 'lawyer' ) ) {
			
			$font_families = array(
				'Playfair+Display:400,400i,700,700i,900',
				'Poppins:100,100i,200i,300,400,500,500i,600,600i,700',
			);
	...
	private function customizer_init(){
		...
		// Instantiate lawyer theme customizer
		$lawyer_theme_customizer = new lawyer_theme_customizer();
	}
	...
	public function set_elementor_flag(){
		update_option( 'lawyer_had_elementor', 'no' );
	}
	// Elementor dsiable default style
	public function elementor_desiable_default_style(){

		$nonce = $_POST['nonce'];
		if ( ! wp_verify_nonce( $nonce, 'lawyer-elementor-notice-nonce' ) ) {
			return;
		}
		$reply = $_POST['reply'];
		if ( ! empty( $reply ) ) {
			...
			update_option( 'lawyer_had_elementor', 'yes' );
		}
		die();
	...
	public function enqueue_elementor_theme_default_style(){
		...
		if ( $disabled_color_schemes === 'yes' && $disabled_typography_schemes === 'yes' ) {
			wp_enqueue_style( 'lawyer-elementor-default-style',  LAWYER_DIR_CSS_URI. 'elementor-default-element-style.css', array(), $this->lawyer_version );
		}
	}
	// Enqueue elementor notice scripts
	public function enqueue_elementor_notice_script(){

		$had_elementor = get_option( 'lawyer_had_elementor' );

		if( $had_elementor == 'no' && self::check_elementor_preview_page() ){
			wp_enqueue_script( 'lawyer-elementor-notice', LAWYER_DIR_JS_URI.'lawyer-elementor-notice.js', array('jquery'), '1.0', true );
			wp_localize_script(
				'lawyer-elementor-notice',
				'lawyerElementorNotice',
				array(
					'ajaxurl' => admin_url( 'admin-ajax.php' ),
					'nonce'   => wp_create_nonce( 'lawyer-elementor-notice-nonce' ),
				)
			);
		}
