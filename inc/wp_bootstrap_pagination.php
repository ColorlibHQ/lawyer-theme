...
function lawyer_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'lawyer' ),
        'next_string'     => esc_html__( '&raquo;', 'lawyer' ),
        'before_output'   => '<div class="lawyer-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'lawyer_pagination_defaults', $defaults )
    );
